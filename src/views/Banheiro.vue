    ...
    <div class="d-flex flex-column" v-if="valoresCorpo">
      ...
      <table class="w-50">
        ...
        <tbody>
          <tr
            class="text-center"
            v-for="(valores, index) in valoresCorpo"
            :key="index"
          >
...
function cortarBanheiro() {
  if (
    (altura.value >= 200) |
    (largura.value >= 200) |
    (comprimento.value >= 300)
  ) {
    const corpoValue = cortarCorpo();
    const tamponamentoValue = cortarTamponamento();
    const portas = cortarPortas();
    return {
      corpoValue,
      tamponamentoValue,
    };
...
function cortarTamponamento() {
  return (valoresTampo.value = [
    {
      comprimento: altura.value,
...
function cortarCorpo() {
  ...
  const prateleiraComp = Math.trunc(
    (comprimento.value - espTamponamentoX2 - espCorpoX3) / (portas.value + 2)
  );
  const travessa = baseComp - espCorpo.value * 2;

  if (nGaveta >= 1) {
    const espacoGavetas =
      comprimento.value - espCorpoX3 - espTamponamentoX2 - prateleiraComp;
    ...
    return (valoresCorpo.value = [
      {
        nome: "Base",
        comprimento: baseComp,
        largura: corpoLarg,
        Quantidade: "1",
        id: 1,
      },
      ...
      {
        nome: "Travessa maior",
        comprimento: travessaMaior,
        largura: larguraTravessa,
        Quantidade: "2",
        id: 5,
      },
      {
        nome: "Travessa menor",
        comprimento: travessaMenor,
        largura: larguraTravessa,
        Quantidade: "2",
        id: 6,
      },
      {
        nome: "Travessa Gaveta",
        comprimento: travessaGavetas,
        largura: alturaGavetas,
        Quantidade: nGavetas,
        id: 7,
      },
      {
        nome: "Lateral Gaveta",
        comprimento: lateraisGavetas,
        largura: alturaGavetas,
        Quantidade: nGavetas,
        id: 8,
      },
...
function cortarPortas() {
  const descontoPortas = 12;
  const alturaPuxador = 35;
  const basePortaGaveta = largura 

  const alturaPorta = altura - descontoPortas - alturaPuxador 
